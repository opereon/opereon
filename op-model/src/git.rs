...
impl GitManager {
    ...
    pub fn get_commit_tree(&mut self, commit: &Sha1Hash) -> GitResult<Tree> {
        let repo = self.open()?;
        let odb = repo.odb().map_err(|err| GitErrorDetail::GetDatabase {err})?;

        let obj = repo
            .find_object(commit.as_oid(), None)
            .map_err(|err| GitErrorDetail::FindObject {err})?;

        let commit_tree = obj.peel_to_tree().map_err(|err| GitErrorDetail::UnexpectedObjectType{err})?;
        Ok(commit_tree)
    }
}